...
### Biological Context Explorer

Select one persisted result in Differential Analysis, Mapping, KEGG, Reactome, STRING, or the Cross-module Explorer and choose **Biological context...**. The new page lists KEGG pathways, Reactome pathways, and STRING neighborhoods from local, frozen runs. Choose a pathway or network before opening a local official image/diagram or building a separate PichAnalysis visualization. Official images are never modified; no network lookup or new STRING expansion is performed.

Generated views show detected members of the selected run: green means up, red means down, and gray means no change or no differential direction. The selected entity has a thicker outline. Direction comes only from a selected frozen Differential run; when none is selected, the view remains neutral. Multiple runs are never resolved by picking the latest automatically. A different or unverified input lineage is offered only as an explicit, caveated biological-identifier match, not as the same experimental feature. Cross-lineage pathway links also require an explicit opt-in. These colors do **not** infer pathway activation, inhibition, or global activity. Generated PNG figures, member tables, and provenance remain in the project and can be reopened from history.
### Experiment Comparison

Analyses → Experiment Comparison compares two independent CSV, TSV, or XLSX tables without replacing the project's main input. A and B may also use the current project dataset or a table frozen by an existing comparison or Differential run. For multi-sheet XLSX files, choose a worksheet for each side. Identifier columns, types, and quantitative-column suggestions require explicit review and confirmation. Choose a comparison identity; unlike exact same-type matching, cross-type matching requires an explicitly selected, persisted Mapping catalog. Ambiguous and unmapped identifiers remain visible rather than being silently matched or discarded.

The Summary, A only, B only, Shared, Ambiguous / Unmapped, Differential behavior, Graphs, Exports, and History tabs display frozen results. Duplicate source rows remain traceable to a single presence entity; UniProt isoforms remain distinct. Protein-group members can contribute to qualitative presence only, never duplicated intensity. If two corresponding Differential Statistics runs are selected, each run's contrast and direction definition remain visible. The user must confirm dataset/run correspondence and separately confirm that the contrasts have comparable biological directions before shared up/down labels are generated. Differential effects, p-values, FDR, and classifications are read from persisted outputs and are never recalculated. Independent LFQ intensities are not divided to infer cross-dataset fold change.

Each comparison freezes both sources and their SHA-256 hashes under `analyses/experiment_comparison/runs/<run_id>`, with per-set CSV files, a consolidated XLSX workbook, simple count graphs, configuration, and manifest. Reopen historical comparisons offline even after the original external files or current project input change. The Functional comparison tab compares explicitly selected, persisted runs from GO (BP/MF/CC separately), KEGG, Reactome, MitoCarta membership/subcompartments/MitoPathways, mtDNA Evidence categories/sources, InterPro/Pfam domains and architectures, Complexes, and STRING nodes/undirected edges. Stable database identifiers take precedence over names, and each side retains its own name, members, counts, enrichment classification, parameters, snapshot and lineage warning. A-only, B-only, shared, enriched-only and enriched-in-both results can be inspected and exported as CSV or a child-run XLSX workbook. Biological Context links for KEGG, Reactome and STRING reuse the existing explorer. Historical functional comparison tables are frozen under the parent comparison run and reopen offline without rerunning their source analyses.

Derived A-only, B-only, Shared and persisted Differential behavior sets can be sent via **Analyze this set...** to GO, KEGG, Reactome, MitoCarta, mtDNA Evidence, InterPro/Pfam, STRING or Complexes when every identifier resolves to the current project's Mapping catalog. The destination opens with a manual target preloaded; the user must review the target, background, snapshot and parameters before starting the analysis. No analysis is run by the handoff. Member counts are descriptive, and neither FDR differences nor pathway membership imply pathway activation, inhibition or other activity.
