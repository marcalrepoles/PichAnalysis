...
Each comparison freezes both sources and their SHA-256 hashes under `analyses/experiment_comparison/runs/<run_id>`, with per-set CSV files, a consolidated XLSX workbook, simple count graphs, configuration, and manifest. Reopen historical comparisons offline even after the original external files or current project input change. This stage does not compare GO, KEGG, Reactome, or other functional annotations.
