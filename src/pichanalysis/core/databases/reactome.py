...
class ReactomeProvider:
 ...
 def download(self,name:str,destination:Path,progress=None)->dict:
  error=None
  for attempt in range(self.retries+1):
   try:
    req=urllib.request.Request(f"{BASE_URL}/{name}",headers={"User-Agent":"PichAnalysis/0.1"});part=destination.with_name(destination.name+".part");digest=hashlib.sha256();size=0
    with urllib.request.urlopen(req,timeout=self.timeout) as response,part.open("wb") as out:
     total=int(response.headers.get("Content-Length",0));
     while chunk:=response.read(1024*1024):out.write(chunk);digest.update(chunk);size+=len(chunk);progress and progress(name,size,total)
    os.replace(part,destination);return {"name":name,"source":f"{BASE_URL}/{name}","file_size":size,"sha256":digest.hexdigest()}
   except (OSError,urllib.error.URLError) as exc:error=exc;time.sleep(min(2**attempt,4))
